"""Prove the MCP stdio handshake. No live EDGAR.

Default: <this python> -m bdc_lookthrough.server
Override: python scripts/smoke_stdio.py -- uvx --from . bdc-lookthrough-mcp
"""
...
EXPECTED_TOOLS = ["get_soi", "loan_overlap", "nonaccrual_names", "reconcile_to_bs"]
...
def smoke(command: Sequence[str], init_timeout: float = 60.0) -> int:
    ...
    try:
        _send(
            proc,
            {
                ...
                "params": {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "bdc-lookthrough-smoke", "version": "0"},
                },
            },
        )
        ...
        if names != EXPECTED_TOOLS:
            ...
            return 1
        if server.get("name") != "bdc-lookthrough":
...
def main(argv: list[str]) -> int:
    if "--" in argv:
        ...
        command = [sys.executable, "-m", "bdc_lookthrough.server"]
